    ...
    <>
      <style>{`
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .meta-group .quote-number { font-size: 22px; font-weight: 700; color: #c9a84c; font-family: 'DM Mono', monospace; }
        ...
        .footer-text { margin-top: 48px; padding-top: 16px; border-top: 1px solid #e5e5e5; font-size: 10px; color: #bbb; text-align: center; font-family: 'DM Mono', monospace; letter-spacing: 1px; line-height: 1.8; }
        .validity-note { margin-top: 24px; padding: 12px 16px; background: #fdf8ee; border-left: 3px solid #c9a84c; font-size: 12px; color: #666; }
      ...
      <div className="page">
        ...
        <div className="meta">
          <div className="meta-group">
            <label>Devis N°</label>
            <span className="quote-number">{quote.number}</span>
          </div>
          <div className="meta-group">
            <label>Client</label>
            <div className="value">{quote.clientName}</div>
            {quote.restaurantName && (
              <div style={{ fontSize: 13, color: '#555', marginTop: 2 }}>{quote.restaurantName}</div>
            )}
            {quote.city && (
              <div style={{ fontSize: 12, color: '#888', marginTop: 1 }}>{quote.city}</div>
            )}
            {quote.clientEmail && (
              <div style={{ fontSize: 12, color: '#888', marginTop: 2 }}>{quote.clientEmail}</div>
            )}
          </div>
          <div className="meta-group" style={{ textAlign: 'right' }}>
            <label>Date</label>
            <div className="value">{new Date(quote.createdAt).toLocaleDateString('fr-CH')}</div>
            {quote.validUntil && (
              <>
                <label style={{ marginTop: 8 }}>Valide jusqu&apos;au</label>
          ...
          <tbody>
            {items.length > 0 ? items.map((item, i) => (
              <tr key={i}>
                ...
                <td className="amount">CHF {(item.quantity * item.unitPrice).toFixed(2)}</td>
              </tr>
            )) : (
        ...
        <div className="totals">
          <div className="total-row"><span>Sous-total HT</span><span>CHF {quote.subtotal.toFixed(2)}</span></div>
          <div className="total-row"><span>TVA ({quote.tva}%)</span><span>CHF {tvaAmount.toFixed(2)}</span></div>
          <div className="total-row final"><span>Total TTC</span><span>CHF {quote.total.toFixed(2)}</span></div>
        </div>

        {quote.notes && (
          <div style={{ marginTop: 32, fontSize: 13, color: '#555', lineHeight: 1.6 }}>
            <div style={{ fontSize: 10, textTransform: 'uppercase', letterSpacing: 2, color: '#999', marginBottom: 6, fontFamily: 'DM Mono, monospace' }}>Notes</div>
            <p>{quote.notes}</p>
          </div>
        )}
        ...
        {quote.validUntil && (
          <div className="validity-note">
            Ce devis est valable jusqu&apos;au {quote.validUntil}. Au-delà de cette date, merci de nous contacter pour un renouvellement.
          </div>
        )}

        <div className="footer-text">
          <p>Altomea · Agence marketing digital pour restaurants · altomea.ch · contact@altomea.ch</p>
        </div>
      </div>
